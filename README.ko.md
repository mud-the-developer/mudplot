...
**엔진 (`mudplot/`) — 지금 바로 사용 가능, 비브라우저 테스트 524개 + 실제 브라우저 테스트 10개 통과:**
...
- [x] 선언적 Spec 모델(`FigureSpec`) + 무손실 JSON 왕복 + 순수 reducer +
      ...
      effect는 가장자리로 분리
- [x] 렌더러: 레이어 27종(line/regplot/scatter/stripplot/bar/errorbar/
      band/stackplot/hline/vline/text/annotate/hist/box/violin/kde/rug/
      hist2d/hexbin/heatmap/contour/contourf/pie/scatter3d/
...
- [x] capabilities 기반 advanced 폼으로 등록된 레이어 27종 모두 지원;
...
2D 필드(heatmap/contour/contourf/hist2d/hexbin), 3D(scatter3d/line3d/surface/wireframe),
