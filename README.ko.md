...
**엔진 (`mudplot/`) — 지금 바로 사용 가능, 비브라우저 테스트 512개 + 실제 브라우저 테스트 10개 통과:**
...
- [x] 선언적 Spec 모델(`FigureSpec`) + 무손실 JSON 왕복 + 순수 reducer +
      ...
      effect는 가장자리로 분리
- [x] 렌더러: 레이어 23종(line/regplot/scatter/bar/errorbar/band/hline/vline/text/
...
- [x] capabilities 기반 advanced 폼으로 등록된 레이어 23종 모두 지원;
...
기본(line/regplot/scatter/bar/errorbar/band), 분포(hist/box/violin/kde/rug),
