...
Uses the physiologically-based model of Machado, Oliveira & Fernandes (2009),
"A Physiologically-based Model for Simulation of Color Vision Deficiency"
(IEEE TVCG). The published 3x3 matrices act on **linear** sRGB and are
tabulated for severities 0.0 .. 1.0 in steps of 0.1; intermediate severities
are linearly interpolated between the two bracketing matrices.
"""
...
__all__ = ["CVD_TYPES", "simulate"]

CVD_TYPES = ("protan", "deutan", "tritan")

# Machado et al. (2009), severities 0.0, 0.1, ..., 1.0 (linear-RGB matrices).
_PROTAN = np.array(
    [
        [
...
_TABLES = {"protan": _PROTAN, "deutan": _DEUTAN, "tritan": _TRITAN}


def _matrix(cvd_type: str, severity: float) -> np.ndarray:
    if cvd_type not in _TABLES:
        raise ValueError(f"cvd_type must be one of {CVD_TYPES}, got {cvd_type!r}")
    severity = float(np.clip(severity, 0.0, 1.0))
    table = _TABLES[cvd_type]
    pos = severity * 10.0
    lo = int(np.floor(pos))
    hi = min(lo + 1, 10)
    frac = pos - lo
...
def simulate(srgb, cvd_type: str = "deutan", severity: float = 1.0) -> np.ndarray:
    """Simulate how ``srgb`` appears under a given colour-vision deficiency.

    Parameters
    ----------
    srgb : array-like, shape (..., 3)
        sRGB colours in [0, 1].
    cvd_type : {'protan', 'deutan', 'tritan'}
    severity : float in [0, 1]
        0 = normal vision, 1 = full dichromacy.

    Returns
    -------
    ndarray, same shape as input, sRGB in [0, 1].
    """
    m = _matrix(cvd_type, severity)
